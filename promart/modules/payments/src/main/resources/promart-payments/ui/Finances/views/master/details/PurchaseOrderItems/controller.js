...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'promart.Finances.PurchaseOrderItems.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('promart.Finances.PurchaseOrderItems.refresh', callback);
		},
		onPurchaseOrdersSelected: function(callback) {
			on('promart.Finances.PurchaseOrders.selected', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '/services/v3/js/promart-payments/api/Finances/PurchaseOrderItems.js';
	...
	$scope.loadPage = function(pageNumber) {
		...
		.success(function(data) {
			...
			$http.get(api + '?PurchaseOrder=' + $scope.masterEntityId + '&$offset=' + ((pageNumber - 1) * $scope.dataLimit) + '&$limit=' + $scope.dataLimit)
			.success(function(data) {
				$scope.data = data;
			});
		});
