...
.factory('$messageHub', [function(){
	...
	var message = function(evtName, data){
		messageHub.post({data: data}, 'promart.Finances.InvoiceItems.' + evtName);
	};
	...
	return {
		...
		onEntityRefresh: function(callback) {
			on('promart.Finances.InvoiceItems.refresh', callback);
		},
		onInvoicesSelected: function(callback) {
			on('promart.Finances.Invoices.selected', callback);
		},
...
.controller('PageController', function ($scope, $http, $messageHub) {
	...
	var api = '/services/v3/js/promart-payments/api/Finances/InvoiceItems.js';
	...
	$scope.loadPage = function(pageNumber) {
		...
		.success(function(data) {
			...
			$http.get(api + '?Invoice=' + $scope.masterEntityId + '&$offset=' + ((pageNumber - 1) * $scope.dataLimit) + '&$limit=' + $scope.dataLimit)
			.success(function(data) {
				$scope.data = data;
			});
		});
	...
	$scope.create = function() {
		$scope.entity.Invoice = $scope.masterEntityId;
	...
	$messageHub.onInvoicesSelected(function(event) {
